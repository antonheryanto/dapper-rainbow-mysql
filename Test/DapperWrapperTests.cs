...
namespace Test
{
	...
	public class DapperWrapperTests
	{
		...
		{
			var objWrapped = new DapperWrapper (new User ());
			Assert.IsTrue (objWrapped.getPrimaryKey() == "Id");

			var objWrapper2 = new DapperWrapper (new Address ());
			Assert.IsTrue (objWrapper2.getPrimaryKey () == "Street");
		}

		[Test]
		public void CanGetPrimaryKeyType(){
			var objWrapper = new DapperWrapper (new User ());
		...
		public void CanGetColumnProperties(){
			var objWrapper = new DapperWrapper (new Address ());
			var columns = objWrapper.getTableColumns ();
			Assert.Equals (columns [0].Name, "Street");
			Assert.Equals (columns [0].Type, typeof(string));
			Assert.Equals (columns.Count, 1);

			var objWrapper2 = new DapperWrapper (new User ());
